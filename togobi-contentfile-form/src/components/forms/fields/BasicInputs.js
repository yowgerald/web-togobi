...
import { appendScript, removeScript } from "../../../Constants";

export const BasicInputs  = (props) => {
    const { contentDetails, handleStatus } = props;
    ...
    useEffect (() => {
        ...
        appendScript(mapApiSource);

        return () => {
            removeScript(initMapUrl);
            removeScript(mapApiSource);
        }
    }, []);
    ...
    return (
        <React.Fragment>
            ...
            <p>
                ...
                <input type="checkbox" name="is_active" id="id_is_active" checked={contentDetails.is_active} onChange={handleStatus}></input>
            </p>
            <p>
